function [Cl, Cd] = ForceCoefficient(Alpha, Re)
%CALCULATE Cl and Cd. 
persistent FL FD
...
if Re < 10000 %Lower Limit of Experimental Data
    Re = 10000;
elseif Re> 6000000 %Upper Limit of Experimental Data
    Re = 6000000;
end
...
if Alpha<=20 && Alpha>=-20
    if isempty(FL) %Check if the fit already exists to avoid repetition
        %IMPORT DATA
        LiftData=dlmread('NACA0012 Lift.txt');
        DragData=dlmread('NACA0012 Drag.txt');
...
if Alpha > 20 || Alpha < -20 || exist('Cd', 'var') == 0 ||isnan(Cd) == 1
    d0 = 1.1657;
    d1 = -1.0058;
    d2 = -0.1253;
...
if Alpha > 20 || Alpha < -20 || exist('Cl', 'var') == 0 || isnan(Cl) == 1
    l0 = 0.1867;
    l1 = 1.4885;
    l2 = 0.1991;
